...
# One grid for the whole line, so a difference between the generations is the
# weights. v3.5-fast is a separate smaller model, not a cheaper mode of v3.5.
TABPFN_VERSIONS: dict[str, str] = {
...
_SERIAL_GRID_MODELS: frozenset[str] = frozenset(TABPFN_VERSIONS) | {"tabicl"}

GRID_SEARCH_MODELS: frozenset[str] = _SERIAL_GRID_MODELS | {"svc", "logreg"}
...
def build_grid_search(
    ...
    if model_name == "svc":
    ...
    elif model_name in TABPFN_VERSIONS:
        ...
        pipeline = TabPFNNativeWrapper(cat_cols=cat_cols,
                                       model_version=TABPFN_VERSIONS[model_name],
                                       random_state=RANDOM_STATE)
        param_grid = {
            "n_estimators":          [4, 8],
